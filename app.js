...
if(!q){document.getElementById("answer").innerHTML="<p>Please describe your project first.</p>";return}
const lower=q.toLowerCase();
let focus=lower.includes("budget")?"Start by fixing a realistic budget range and keep a contingency reserve.": "Start by defining plot size, location, floors, rooms, budget and intended use.";
let stages=["1. Confirm plot dimensions, ownership documents and local planning constraints.","2. Get soil testing and discuss the site with a qualified civil/structural engineer.","3. Prepare an architectural concept and room requirements.","4. Develop structural design and a detailed quantity/cost estimate.","5. Check required local permissions before construction.","6. Compare contractor/material quotations and define a project schedule.","7. Track changes, bills and important documents throughout construction."];
if(lower.includes("1200")) stages.splice(0,0,"Project note: for a 1200 sq.ft plot, first confirm setbacks and permissible built-up area before assuming the full plot can be built.");
document.getElementById("answer").innerHTML="<h3>BuildWise preliminary guidance</h3><p><b>First priority:</b> "+focus+"</p><ol>"+stages.map(x=>"<li>"+x+"</li>").join("")+"</ol><p class='ai-note'>Prototype guidance only. A qualified professional should verify site-specific engineering, legal and cost decisions.</p>";
}
